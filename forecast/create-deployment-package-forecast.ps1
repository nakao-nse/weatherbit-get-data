<#
.SYNOPSIS
    Forecast用Lambdaデプロイパッケージ作成スクリプト
    
.DESCRIPTION
    このスクリプトはForecast用Lambda関数のデプロイパッケージ（zipファイル）を作成します。
    Lambda関数のコードファイルと依存パッケージを含むzipファイルを生成します。
    
.PARAMETER OutputZip
    出力するzipファイル名（デフォルト: lambda-deployment-forecast.zip）
...
.EXAMPLE
    .\create-deployment-package-forecast.ps1
    
.EXAMPLE
    .\create-deployment-package-forecast.ps1 -OutputZip "my-forecast-lambda-package.zip"
    
.EXAMPLE
    .\create-deployment-package-forecast.ps1 -PythonVersion "python3" -Cleanup:$false
    
.EXAMPLE
    .\create-deployment-package-forecast.ps1 -UseUv:$false
...
param(
    [string]$OutputZip = "lambda-deployment-forecast.zip",
...
Write-Host "Forecast用Lambdaデプロイパッケージ作成スクリプト" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
# 一時ディレクトリを作成
$TempDir = Join-Path $env:TEMP "lambda-deployment-forecast-$(Get-Date -Format 'yyyyMMddHHmmss')"
Write-Host "一時ディレクトリを作成: $TempDir" -ForegroundColor Yellow
New-Item -ItemType Directory -Path $TempDir -Force | Out-Null

try {
    # Lambda関数のコードファイルをコピー
    Write-Host "`nForecast用Lambda関数のコードファイルをコピー中..." -ForegroundColor Yellow
    $CodeFiles = @(
        "lambda_function_forecast.py",
        "config_forecast.py",
        "weatherbit_client_forecast.py",
        "csv_converter_forecast.py",
        "s3_handler_forecast.py",
        "local_handler_forecast.py"
    )
    ...
    if ($useUvForInstall) {
        ...
        if (-not $pythonCmd) {
            ...
            Write-Host "例: .\create-deployment-package-forecast.ps1 -PythonVersion python3" -ForegroundColor Yellow
            exit 1
        }
