    ...
    def __init__(self):
        ...
        if is_local:
            ...
            self.s3_prefix = os.getenv(
                self.S3_PREFIX,
                config_data.get("S3_PREFIX", "weather-data-forecast")
            )
        else:
            # AWS実行時は必須
        ...
        # Forecast Hours: 環境変数 → config.json → デフォルト値
        forecast_hours_str = os.getenv(
            self.FORECAST_HOURS,
            config_data.get("FORECAST_HOURS", str(self.DEFAULT_FORECAST_HOURS))
        )
        try:
            self.forecast_hours = int(forecast_hours_str)
            if self.forecast_hours <= 0 or self.forecast_hours > 240:
                raise ValueError(f"FORECAST_HOURS must be between 1 and 240, got {self.forecast_hours}")
        except ValueError as e:
            if "must be between" in str(e):
                raise
            raise ValueError(f"Invalid FORECAST_HOURS value: {forecast_hours_str}")
